...
export async function POST(request: NextRequest) {
  try {
    ...
    const userId = (inputs as any).user_id;
    if (userId) {
      // Calculate water saved (if user is below average, they're saving water)
      const regionalAverage = getRegionalAverage(location);
      const perPersonDaily = dailyLitersPerPerson;
      if (perPersonDaily < regionalAverage) {
        const waterSaved = (regionalAverage - perPersonDaily) * 365; // Annual savings
        trackCalculatorUsage(userId, "water", waterSaved, location).catch(console.error);
      } else {
        // Still reward for using the tool
        trackCalculatorUsage(userId, "water", dailyLiters, location).catch(console.error);
      }
    }
